...
type InfectNearbyOptions = Options & {
    action: ExecutorActions,
    count: number,
}
...
export class Executor {
    ...
    public run = (args: RunArgs) => {
        ...
        if (!host || !action || !target) {
            this.log(`Invalid arguments: [${JSON.stringify({ host, action, target })}]`)
            return
        }
        ...
        try {
            switch (action) {
                case ExecutorActions.grow: {
                    ...
                    this.log(`Attempting to grow target [${options.target}] from host [${options.host}]...`)
                ...
                case ExecutorActions.infect: {
                    if (isValidInfectArgs()) {
                        const infectAction = args[3] as ExecutorActions
                        const options: InfectNearbyOptions = { host, target, action: infectAction, count: Number(args[4]) }

                        if (options.count > 1) {
                            this.log(`Attempting to infect ${options.count} server(s) nearby host [${options.host}]`)
                        } else {
                            this.log(`Attempting to infect host [${options.host}]`)
                        }

                        this.log(`Infected will execute action [${options.action}] targeting [${options.target}]`)
                        this.infectNearby(options)
                    } else {
                        this.log(`Invalid arguments for infect action: [${args[3]}, ${args[4]}]`)
                    }
            ...
            this.log(`Error executing action [${action}] on host [${host}] targeting [${target}]: ${error?.toString()}`)
        }
    }
    ...
    public growTarget = (options: Options) => {
        ...
        const threadsToUse = options.threads || this.getMaxThreads(hostServer, pathToScript)

        if (threadsToUse <= 0) {
            throw new Error(`Not enough threads available on ${options.host} to grow ${target}...`)
        }

        this.log(`Growing ${target} using ${threadsToUse} thread(s) on ${options.host}...`)
        const pid = this.ns.exec(pathToScript, options.host, threadsToUse, target)
    ...
    public harvestTarget = (options: Options) => {
        ...
        if (threadsToUse <= 0) {
            throw new Error(`Not enough threads available on ${options.host} to harvest ${target}...`)
        }

        this.log(`Harvesting ${target} using ${threadsToUse} thread(s) on ${options.host}...`)
        const pid = this.ns.exec(pathToScript, options.host, threadsToUse, target)
        this.log(`Executed with PID: ${pid}`)
    }
    ...
    public infectNearby = (options: InfectNearbyOptions) => {
        if (options.action === ExecutorActions.infect) {
            ...
            throw new Error(`Cannot infect nearby servers with infected action as [${options.action}]`)
        }
        ...
        try {
            ...
            const hostServer = this.ns.getServer(options.host)
            const target = options.target || hostServer.hostname
            const threadsToUse = options.threads || this.getMinThreads(hostServer, pathToScript)

            if (threadsToUse <= 0) {
                throw new Error(`Not enough threads available on ${options.host} to infect nearby servers...`)
            }

            const pid = this.ns.exec(pathToScript, options.host, threadsToUse, target, options.action, options.count)
            this.log(`Executed with PID: ${pid}`)
        } catch (error) {
    ...
    private getMaxThreads = (server: Server, pathToScript: string): number => {
        ...
        this.log(`Checking if host [${server.hostname}] can run [${pathToScript}]`)
